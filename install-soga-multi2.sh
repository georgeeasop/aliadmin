...
config_instance_custom() {
    ...
    read -p "请输入 server_type (默认: ss；soga3 建议 anytls): " input_server_type
    if [[ "$instance_name" == "soga3" ]]; then
    ...
    if [[ "$instance_name" == "soga3" ]]; then
        echo ""
        read -p "请输入 cert_domain (默认: vowa88.top): " input_cert_domain
        input_cert_domain=${input_cert_domain:-vowa88.top}

        read -p "请输入 cert_mode (默认: dns): " input_cert_mode
        input_cert_mode=${input_cert_mode:-dns}

        read -p "请输入 cert_key_length (默认: ec-256): " input_cert_key_length
        input_cert_key_length=${input_cert_key_length:-ec-256}

        read -p "请输入 dns_provider (默认: dns_cf): " input_dns_provider
        input_dns_provider=${input_dns_provider:-dns_cf}

        read -p "请输入 DNS_CF_Email (默认: ruopoppy@gmail.com): " input_dns_cf_email
        input_dns_cf_email=${input_dns_cf_email:-ruopoppy@gmail.com}
...
main() {
    ...
    while true; do
        ...
        99)
            ...
            ;;
        *)
            echo -e "${red}请输入正确的数字 [0-33, 99]${plain}"
            sleep 2
            ;;
        esac
    done
