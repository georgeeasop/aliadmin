...
SOGA_DEFAULT_LOG_LEVEL="error"
SOGA_DEFAULT_LOG_FILE_DIR="false"
SOGA_DEFAULT_LOG_FILE_RETENTION_DAYS="2"
...
SOGA2_DEFAULT_LOG_LEVEL="error"
SOGA2_DEFAULT_LOG_FILE_DIR="false"
SOGA2_DEFAULT_LOG_FILE_RETENTION_DAYS="2"

# soga3 默认配置（除 node_id 外，node_id 需要用户输入）
SOGA3_DEFAULT_TYPE="xboard"
SOGA3_DEFAULT_SERVER_TYPE="anytls"
SOGA3_DEFAULT_SOGA_KEY="5SrOk5VxovqomAVgKAIKBXGednyRpMSw"
SOGA3_DEFAULT_WEBAPI_URL="https://vowa.top/"
SOGA3_DEFAULT_WEBAPI_KEY="M2X84M6a7N0iGHWC8fU7p8bwrVcCBmz"
SOGA3_DEFAULT_CERT_DOMAIN="vowa88.top"
SOGA3_DEFAULT_CERT_MODE="dns"
SOGA3_DEFAULT_CERT_KEY_LENGTH="ec-256"
SOGA3_DEFAULT_DNS_PROVIDER="dns_cf"
SOGA3_DEFAULT_DNS_CF_EMAIL="ruopoppy@gmail.com"
SOGA3_DEFAULT_DNS_CF_KEY="cfk_or4TzZZZVTDA2Clij3l7gfgUCQeXnGigfmJehdpt6bafa35a"
SOGA3_DEFAULT_LOG_LEVEL="error"
SOGA3_DEFAULT_LOG_FILE_DIR="false"
SOGA3_DEFAULT_LOG_FILE_RETENTION_DAYS="2"

# soga4 默认配置（除 node_id 外，node_id 需要用户输入）
SOGA4_DEFAULT_TYPE="xboard"
SOGA4_DEFAULT_SERVER_TYPE="anytls"
SOGA4_DEFAULT_SOGA_KEY="HD7p1XpcTOLgaszkzg4fV2LgfZtYaIXK"
SOGA4_DEFAULT_WEBAPI_URL="https://mexta.top/"
SOGA4_DEFAULT_WEBAPI_KEY="1uU01fp9vUYZ7MrJdiSUee"
SOGA4_DEFAULT_CERT_DOMAIN="mexta.click"
SOGA4_DEFAULT_CERT_MODE="dns"
SOGA4_DEFAULT_CERT_KEY_LENGTH="ec-256"
SOGA4_DEFAULT_DNS_PROVIDER="dns_cf"
SOGA4_DEFAULT_DNS_CF_EMAIL="georgeeasop@gmail.com"
SOGA4_DEFAULT_DNS_CF_KEY="cfk_Oh2xAqSfDd6hCeTKOtMKrNZlLJcMpOzN44OCucLk1cbbb9ac"
SOGA4_DEFAULT_LOG_LEVEL="error"
SOGA4_DEFAULT_LOG_FILE_DIR="false"
SOGA4_DEFAULT_LOG_FILE_RETENTION_DAYS="2"
...
is_instance_running() {
    ...
    # 检查 screen 窗口是否运行
    if screen -list 2>/dev/null | grep -qw "${instance_name}"; then
        return 0
    fi
...
install_soga_instance() {
    ...
    for i in soga soga2 soga3 soga4; do
        if [[ "$i" != "$instance_name" ]]; then
            ...
            if [[ -d "$other_config" ]] && [[ -f "$other_config/soga.conf" ]]; then
                other_instances+=("$i")
                local run_mode=""
                if screen -list 2>/dev/null | grep -qw "${i}"; then
                    run_mode="(screen)"
    ...
    if [[ $has_config -eq 1 ]]; then
        ...
        if [[ "$start_now" == "y" || "$start_now" == "Y" ]]; then
            ...
            echo -e "${yellow}配置命令: 使用脚本菜单选项 5-7、34（默认配置）或 33（自定义配置）${plain}"
...
config_instance_default() {
    ...
    local default_log_file_dir=""
    local default_log_file_retention_days=""
    local default_cert_domain=""
    local default_cert_mode=""
    local default_cert_key_length=""
    local default_dns_provider=""
    local default_dns_cf_email=""
    local default_dns_cf_key=""
    ...
    if [[ "$instance_name" == "soga" ]]; then
        ...
        default_log_level=${SOGA_DEFAULT_LOG_LEVEL}
        default_log_file_dir=${SOGA_DEFAULT_LOG_FILE_DIR}
        default_log_file_retention_days=${SOGA_DEFAULT_LOG_FILE_RETENTION_DAYS}
        ...
        default_log_level=${SOGA2_DEFAULT_LOG_LEVEL}
        default_log_file_dir=${SOGA2_DEFAULT_LOG_FILE_DIR}
        default_log_file_retention_days=${SOGA2_DEFAULT_LOG_FILE_RETENTION_DAYS}
    elif [[ "$instance_name" == "soga3" ]]; then
        default_type=${SOGA3_DEFAULT_TYPE}
        default_server_type=${SOGA3_DEFAULT_SERVER_TYPE}
        default_soga_key=${SOGA3_DEFAULT_SOGA_KEY}
        default_webapi_url=${SOGA3_DEFAULT_WEBAPI_URL}
        default_webapi_key=${SOGA3_DEFAULT_WEBAPI_KEY}
        default_log_level=${SOGA3_DEFAULT_LOG_LEVEL}
        default_log_file_dir=${SOGA3_DEFAULT_LOG_FILE_DIR}
        default_log_file_retention_days=${SOGA3_DEFAULT_LOG_FILE_RETENTION_DAYS}
        default_cert_domain=${SOGA3_DEFAULT_CERT_DOMAIN}
        default_cert_mode=${SOGA3_DEFAULT_CERT_MODE}
        default_cert_key_length=${SOGA3_DEFAULT_CERT_KEY_LENGTH}
        default_dns_provider=${SOGA3_DEFAULT_DNS_PROVIDER}
        default_dns_cf_email=${SOGA3_DEFAULT_DNS_CF_EMAIL}
        default_dns_cf_key=${SOGA3_DEFAULT_DNS_CF_KEY}
    elif [[ "$instance_name" == "soga4" ]]; then
        default_type=${SOGA4_DEFAULT_TYPE}
        default_server_type=${SOGA4_DEFAULT_SERVER_TYPE}
        default_soga_key=${SOGA4_DEFAULT_SOGA_KEY}
        default_webapi_url=${SOGA4_DEFAULT_WEBAPI_URL}
        default_webapi_key=${SOGA4_DEFAULT_WEBAPI_KEY}
        default_log_level=${SOGA4_DEFAULT_LOG_LEVEL}
        default_log_file_dir=${SOGA4_DEFAULT_LOG_FILE_DIR}
        default_log_file_retention_days=${SOGA4_DEFAULT_LOG_FILE_RETENTION_DAYS}
        default_cert_domain=${SOGA4_DEFAULT_CERT_DOMAIN}
        default_cert_mode=${SOGA4_DEFAULT_CERT_MODE}
        default_cert_key_length=${SOGA4_DEFAULT_CERT_KEY_LENGTH}
        default_dns_provider=${SOGA4_DEFAULT_DNS_PROVIDER}
        default_dns_cf_email=${SOGA4_DEFAULT_DNS_CF_EMAIL}
        default_dns_cf_key=${SOGA4_DEFAULT_DNS_CF_KEY}
    else
    ...
    # 附加日志相关配置（默认 log_level=error、log_file_dir=false、log_file_retention_days=2，避免产生日志文件）
    if [[ -n "$default_log_level" ]]; then
        default_config="${default_config} log_level=${default_log_level}"
    fi
    if [[ -n "$default_log_file_dir" ]]; then
        default_config="${default_config} log_file_dir=${default_log_file_dir}"
    fi
    if [[ -n "$default_log_file_retention_days" ]]; then
        default_config="${default_config} log_file_retention_days=${default_log_file_retention_days}"
    fi

    # soga3/soga4 附加自动证书配置
    if [[ "$instance_name" == "soga3" || "$instance_name" == "soga4" ]]; then
        default_config="${default_config} cert_domain=${default_cert_domain} cert_mode=${default_cert_mode} cert_key_length=${default_cert_key_length} dns_provider=${default_dns_provider} DNS_CF_Email=${default_dns_cf_email} DNS_CF_Key=${default_dns_cf_key}"
    fi
    ...
    # 直接写入配置文件（可靠，不依赖 soga config 子命令是否支持所有键）
    ...
    if [[ "$start_now" == "y" || "$start_now" == "Y" ]]; then
        # 如果已在运行，先停止（内部调用，跳过 wait_for_enter）
        if is_instance_running ${instance_name}; then
            stop_instance ${instance_name} 1
            sleep 1
        fi
        start_instance ${instance_name}
    else
...
config_instance_custom() {
    ...
    read -p "请输入 server_type (默认: ss；soga3/soga4 建议 anytls): " input_server_type
    if [[ "$instance_name" == "soga3" || "$instance_name" == "soga4" ]]; then
        input_server_type=${input_server_type:-anytls}
    else
    ...
    if [[ "$instance_name" == "soga3" || "$instance_name" == "soga4" ]]; then
        ...
        local cert_domain_default="vowa88.top"
        local cert_mode_default="dns"
        local cert_key_length_default="ec-256"
        local dns_provider_default="dns_cf"
        local dns_cf_email_default="ruopoppy@gmail.com"

        if [[ "$instance_name" == "soga4" ]]; then
            cert_domain_default="mexta.click"
            cert_mode_default="dns"
            cert_key_length_default="ec-256"
            dns_provider_default="dns_cf"
            dns_cf_email_default="georgeeasop@gmail.com"
        fi

        read -p "请输入 cert_domain (默认: ${cert_domain_default}): " input_cert_domain
        input_cert_domain=${input_cert_domain:-$cert_domain_default}

        read -p "请输入 cert_mode (默认: ${cert_mode_default}): " input_cert_mode
        input_cert_mode=${input_cert_mode:-$cert_mode_default}

        read -p "请输入 cert_key_length (默认: ${cert_key_length_default}): " input_cert_key_length
        input_cert_key_length=${input_cert_key_length:-$cert_key_length_default}

        read -p "请输入 dns_provider (默认: ${dns_provider_default}): " input_dns_provider
        input_dns_provider=${input_dns_provider:-$dns_provider_default}

        read -p "请输入 DNS_CF_Email (默认: ${dns_cf_email_default}): " input_dns_cf_email
        input_dns_cf_email=${input_dns_cf_email:-$dns_cf_email_default}

        read -p "请输入 DNS_CF_Key: " input_dns_cf_key
        if [[ -z "$input_dns_cf_key" ]]; then
            echo -e "${red}DNS_CF_Key 不能为空！${plain}"
            wait_for_enter
            return 1
        fi

        custom_config="${custom_config} cert_domain=${input_cert_domain} cert_mode=${input_cert_mode} cert_key_length=${input_cert_key_length} dns_provider=${input_dns_provider} DNS_CF_Email=${input_dns_cf_email} DNS_CF_Key=${input_dns_cf_key}"
    fi
...
# 启动实例（使用 screen）（no_wait=1 时跳过 wait_for_enter，供内部调用）
start_instance() {
    local instance_name=$1
    local no_wait=${2:-0}
    ...
    if [[ ! -s ${config_dir}/soga.conf ]] || ! grep -qE "^(type|soga_key|webapi)" ${config_dir}/soga.conf 2>/dev/null; then
        ...
        echo -e "${yellow}请先配置 ${instance_name}（使用脚本菜单选项 5-7、34 或 33）${plain}"
        [[ "$no_wait" != "1" ]] && wait_for_enter
        return 1
    fi
...
# 停止实例（no_wait=1 时跳过 wait_for_enter，供内部调用）
stop_instance() {
    local instance_name=$1
    local no_wait=${2:-0}
...
uninstall_instance() {
    ...
    # 停止运行中的实例（内部调用，跳过 wait_for_enter）
    if is_instance_running ${instance_name}; then
        stop_instance ${instance_name} 1
    fi
...
show_main_menu() {
    ...
    echo -e "  ${green}7.${plain} 配置默认 soga3 配置"
    echo -e "  ${green}34.${plain} 配置默认 soga4 配置"
    echo -e "  ${green}33.${plain} 自定义配置"
    echo ""
    ...
    echo -e "${yellow}      使用 'screen -r soga'、'screen -r soga2'、'screen -r soga3'、'screen -r soga4' 查看日志${plain}"
    echo ""
}

# 主逻辑
main() {
    # 本脚本使用 screen 后台运行，不强制依赖 systemctl
    # （仅在兼容旧 systemd 实例时会用到，缺失时相关检查自动跳过）

    while true; do
        show_main_menu
        read -p "请选择 [0-34, 99]: " choice
        ...
        6)
            ...
            ;;
        7)
            config_instance_default "soga3"
            ;;
        34)
            config_instance_default "soga4"
            ;;
        33)
            echo ""
        ...
        99)
            ...
            ;;
        *)
            echo -e "${red}请输入正确的数字 [0-34, 99]${plain}"
            sleep 2
            ;;
        esac
    done
